...
const renderCourseDetail = (course, writable) => {
  ...
  return '<section class="school-course-plan' + courseToneClass(course) + '">' +
    '<header class="school-course-plan-header"><button type="button" data-action="course-back">← Ukeplan</button><div><p class="eyebrow">' + escapeHtml(course.code) + '</p><h2>' + escapeHtml(course.name) + '</h2><span>' + escapeHtml(course.workMode + " · " + course.status) + '</span></div></header>' +
